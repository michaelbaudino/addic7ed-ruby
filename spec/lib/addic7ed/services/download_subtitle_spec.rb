...
describe Addic7ed::DownloadSubtitle do
  ...
  describe "#call" do
    let!(:http_stub) { stub_request(:get, url).to_return File.new("spec/responses/walking-dead-3-2-8_best_subtitle.http") }
    ...
    context "when the HTTP request returns a HTTP redirection" do
      let!(:http_stub)     { stub_request(:get, url).to_return File.new("spec/responses/basic_redirection.http") }
      let!(:redirect_url)  { "http://www.addic7ed.com/original/68018/4.redirected" }
      let!(:redirect_stub) { stub_request(:get, redirect_url).to_return File.new("spec/responses/walking-dead-3-2-8_best_subtitle.http") }
      ...
      context "when it gets stuck in a redirection loop" do
        let!(:redirect_stub) { stub_request(:get, redirect_url).to_return File.new("spec/responses/basic_redirection.http") }

        it "follows up to HTTP_REDIRECT_LIMIT redirections then raises a Addic7ed::DownloadError" do
          expect(described_class).to receive(:new).exactly(described_class.const_get(:HTTP_REDIRECT_LIMIT) + 1).times.and_call_original
